...
class ZipArchiveReadingTestAbsPath(ZipArchiveReadingTest):
    ...
    def test_open_repr(self):
        self.assertTrue(repr(self.c).startswith("<open Collection '/vsizip/"))

    def test_closed_repr(self):
        self.c.close()
        self.assertTrue(repr(self.c).startswith("<closed Collection '/vsizip/"))

    def test_path(self):
        self.assertTrue(self.c.path.startswith('/vsizip/'))
